...
export function ShareButton({ entityType, entityId, entityTitle }: ShareButtonProps) {
  const { createShareLink, getShareUrl } = useShareLinks();
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-[#1a1a1a] border border-[#C4A264]/30 rounded-lg w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-[#F0EAE0]">Partager {entityTitle || entityType}</h3>
              <button onClick={() => { setIsOpen(false); setShareUrl(null); }} className="text-[#F0EAE0]/50 hover:text-[#F0EAE0]">
                <X className="w-5 h-5" />
              </button>
            </div>
